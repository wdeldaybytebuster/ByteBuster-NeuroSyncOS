> Archived verbatim 2026-07-08 from
> `docs/docs/05-delivery/implementation-plan-reliability.md`, which actively
> contradicted the real codebase (Node.js 24 as "mandatory" — real requirement
> is 22+; flat `/src/coreexec/` paths — real layout is `src/core/coreexec/`;
> "Phase 1 (Current)" framing predates the 2026-06-30 reality audit). Several
> specifics here are real and fact-checked into current docs: the
> `CommandSandbox` allowlist → `docs/docs/04-security/security-privacy-model.md`,
> SA-01–SA-06 → same file, the redaction pipeline and habituation model →
> `docs/docs/02-architecture/data-architecture.md`. For current delivery
> status use `docs/implementation-plan-and-progress-tracker.md`, not this
> file's phase/unit numbering.

...
- Browser executable paths explicitly target `/usr/bin/google-chrome-beta`.
- Environment variables limit memory footprint to 1GB and thread pool to 3.
- Running `npm run build` cleanly compiles the frontend.
- Running `npm start` launches a single Node process on Port 3743 that serves both the UI and the API Gateway.

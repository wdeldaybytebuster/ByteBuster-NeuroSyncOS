...
| 12 | Land OpenCode Zen/OpenRouter provider registry + real llama-cpp inference (pre-existing uncommitted work) | — | VERIFIED (tests + typecheck + build) | 2026-07-03 |
| 13 | Fix ProjectSwitcher hydration warning, CPU-temp fail-open-to-0°C, dead Decline/Reject buttons | P0 | VERIFIED (tests + typecheck + build) | 2026-07-03 |
| 14 | Project archive/restore endpoints + persist active project selection | P1 | VERIFIED (tests + typecheck + build + live-DB cleanup) | 2026-07-03 |
| 15 | Replace fabricated dashboard telemetry (fake test counts, "Pino" log, static confidence) with real aggregates | P1 | VERIFIED (tests + typecheck + build) | 2026-07-03 |
| 16 | Unify `os_todos` + DAG-proposal approval queues over one `dag_proposals` table | P0 | VERIFIED (tests + typecheck + build) — done by Opus per plan | 2026-07-03 |
| 17 | Auto-scan project docs when a PortGrid terminal session closes (learning loop) | P1 | VERIFIED (tests + typecheck + build) | 2026-07-03 |
| 18 | Bind current Node bin dir into terminal sandbox (nvm-installed CLI reachability) | P1 | VERIFIED — `claude` blocked by unbound `lib/` symlink target at first landing; fully resolved by Task 20 below same day | 2026-07-03 |
| 19 | Live-test OpenCode Zen + OpenRouter with real user-provided keys via UI | — | VERIFIED (real chat + DAG generation exercised; keys applied to both the worktree test copy and the user's real dev DB) | 2026-07-03 |
| 20 | Restore terminal network (remove `--unshare-net`) + bind ALL 5 CLI toolchains (claude, codex, agy, opencode, freebuff) | P0 | VERIFIED (tests + typecheck + build + live: `claude --version` launches, real HTTPS request succeeds) — done by Opus per explicit user security sign-off | 2026-07-03 |
| 21 | Make ScopeLogic DAG proposal generation LLM-driven with real self-reported confidence + scope sessions per-project | P1 | VERIFIED (tests + typecheck + build) — done by Opus per plan | 2026-07-03 |
| 22 | Wire up real Log Level enforcement (shared logger utility, 9 highest-traffic files migrated) | P2 | VERIFIED (tests + typecheck + build + live: real pre-existing 'warn' setting now actually suppresses info logs) | 2026-07-03 |
...
### 2026-07-03 — Dashboard reality pass: fabricated telemetry, project cleanup, unified approvals, terminal learning loop

Trigger: a request to re-review every dashboard against real hands-on usage (project management, agent creation/scheduling, using the embedded terminal to have a coding agent make changes) rather than re-reading source. A testing pass against the live app (real LLM provider, real DAG execution) confirmed the core orchestration loop genuinely works end-to-end, but surfaced concrete problems beyond what the 06-30 audit's 11 tasks had already closed. Full plan and design-decision record: see the approved plan this session worked from (project management archive/persistence, honest telemetry, approval-system unification, terminal learning loop + sandboxed PATH widening). Work happened in an isolated worktree (`worktree-dashboard-reality-pass`), six commits, all summarized below — read each commit's own body for full detail, this is a summary index.

**Task 12 — landed pre-existing uncommitted work (`dcb3a6b`):** the session found the `oss-readiness` branch's working tree already had a fully-tested, documented-but-uncommitted feature (OpenCode Zen / OpenRouter as first-class LLM providers, real `node-llama-cpp` inference replacing a stub, several live-tested bug fixes, test-DB isolation) sitting on top of it. Verified `npm test`/`tsc`/`build` clean and `detect_changes` matched the expected surface (llama-cpp, mock-provider, openai-compatible, discovery, RouteSwitchDashboard) before committing it as a clean baseline. See `docs/llm-provider-testing-plan-2026-07-01.md` for the full detail of what that work contains — its own Phase 1 live-test matrix is still open and needs a human with real API keys, not something an unattended session can complete.

**Task 13 — quick real bugs (`fbc45d2`):** `ProjectSwitcher.tsx` had a `<button>` nested inside a `<button>` (a React hydration-warning-producing anti-pattern) — the outer row is now `div[role=button]` with keyboard support. `system.ts`'s `/api/system/metrics` SSE stream did `temperature: temp.main || 0`, silently turning a failed/unavailable CPU sensor read into a fake `0.0°C`; now passes `null` through and `GovernorUI.tsx` shows "Unavailable". The single-item "Decline" button in PortGrid's approval queue had no `onClick` at all, and the Deference UI pill bar's "Reject All" only cleared local state with no backend effect; added real `POST /api/todos/reject` and `/reject-bulk` (flips `status` to `'rejected'`, deliberately does NOT unclaim the task/resume the run, unlike `/resolve`) and wired both buttons. Added `todos.test.ts` (didn't exist before, despite `/promote`/`/resolve`/`/resolve-bulk` already existing).

**Task 14 — project archive/restore + persistence (`b8fb59b`):** there was no delete/archive endpoint for projects at all — confirmed via hands-on testing that the live DB already had 10 leftover adversarial-test-fixture project rows (`__BLOCKED_BY_VALIDATION__` ×6, `rogue-agent`, `reserved-label`, `empty-nodes`, `bad-json`, all `workspace_path`/`project_root_path` NULL, all created in the same millisecond batch — almost certainly from `coreexec-router.test.ts`-style tests that ran before this session's earlier `:memory:` DB isolation fix landed) cluttering the switcher forever with zero cleanup path. Added a nullable `projects.archived_at` column (soft-delete), `POST /:id/archive` + `/:id/restore`, `GET /?includeArchived=true`, and an Archive action in `ProjectSwitcher.tsx`. Used the new endpoint to archive all 10 fixture rows directly against the live `.data/neurosync.db` (added the column there by hand first, since the live dev server was still running old code) — only the real `BBR Pipeline` project remains active. Also fixed: active project selection was pure in-memory React state that reset to "Global" on every reload; now persists via `localStorage` (`ns-active-project`), mirroring `ThemeContext.tsx`'s pattern.

**Task 15 — honest dashboards (`e4f0caa`):** `CoreExecDashboard.tsx`, `ScopeLogicDashboard.tsx`, and `UnifiedMasterDashboard.tsx` all showed hardcoded strings dressed up as live telemetry — "398 Tests / PASSING", "Duplicates 0", "Retry Rate 1.2%", "Latency 42ms", a "Pino Transaction Log" heading (zero Pino dependency anywhere in this repo), a fake prompt version "v3.2.1" with a fake timestamp, and a static 98.4% "Multi-Model Consensus Rating" that was never actually computed. Added a real `completed_at` column to `workflow_runs` (stamped on every terminal transition, cleared on retry) and a new `GET /api/coreexec/metrics` endpoint aggregating real success rate / active runs / retry rate (resolved `os_todos` escalations ÷ total tasks) / avg latency from `workflow_runs`/`tasks`/`os_todos`. Added `GET /api/scopelogic/prompt-info` exposing the real file mtime of `interview.ts` (there is no real version-numbering system for the prompt — it's a plain string constant) in place of the fabricated version/test-count. **New finding surfaced while fixing this:** `interview.ts`'s `_generateProposal()` is template-based, not an LLM call at all — the conversational Q&A genuinely uses the LLM, but the final DAG proposal structure is built by truncating user message substrings into a fixed 4-node template. This is why there was no real confidence score to show; flagged as a separate, deliberately-not-fixed architectural gap (proposal generation not being LLM-driven) that needs its own decision, not folded into this pass. Also found (separately) that `UnifiedMasterDashboard.tsx`'s "Log Level" control persists a setting to `system_settings` that is never read anywhere in the backend — fixed the misleading "(Pino)" heading but did not build real log-level enforcement (out of scope, flagged only).

**Task 16 — unified approval system, done by an Opus agent per the plan's routing (`facdc1a`):** PortGrid had two disconnected "pending approval" mechanisms — `os_todos` (numeric confidence, real table) and a whole-DAG-proposal blob crammed into `system_settings['pending_proposal']` (no confidence, no table, separate endpoints) — confirmed the HITL widget's "All Clear" message could show while a real proposal sat pending in a different widget on the same screen. Added a real `dag_proposals` table (`id, project_id, proposal, confidence REAL DEFAULT 0.5, status, created_at`), a migration for any legacy blob already in `system_settings`, and merged PortGrid's rendering (`src/ui/lib/approvalQueue.ts`'s `buildApprovalItems`/`partitionByConfidence`/`partitionByKind`) so both sources flow through the same 0.70-threshold Deference UI split. New proposals default to confidence 0.5 (always manual review) since — per Task 15's finding — there's no real per-proposal AI confidence signal to derive from today; documented as a deliberate, conservative default, not a placeholder to forget about. +22 tests. See the commit body for the full design-decision record (single-active-proposal choice, mixed-kind bulk-action routing, what was found but not fixed).

**Task 17 — terminal learning loop (`4e276e6`):** the embedded terminal lets a real coding agent (`opencode`, confirmed working) change project files, with zero automatic path back into NeuroSync's own memory system — `/api/okf/scan-project` was manual-click-only. Extracted the doc-discovery walk out of that route into `src/core/okf/project-scanner.ts`'s `scanProjectForDocs()` so it's callable in-process, then wired it into `TerminalSession.dispose()` (the one lifecycle event every close path — WS close, WS error, shell exit — already funnels through), with a 60s per-project cooldown and best-effort error handling that never blocks terminal teardown. Broadcasts a `TERMINAL_AUTO_SCAN` event over the existing ScoutDaemon SSE channel so `PortGridDashboard.tsx` can show a dismissing "Project scanned..." banner — visible feedback, not silent magic. Scope is deliberately minimal: this only refreshes the discovered/unprocessed doc list, actually converting docs into OKF concepts is still the separate explicit `/convert-document` step.

**Task 18 — sandbox PATH widening, done by an Opus agent per the plan's routing, with an explicit security self-review (`5bac029`):** PATH inside the terminal sandbox was cleared to system dirs only, so CLI tools installed under an nvm-managed Node (including `claude` itself on this dev machine) didn't resolve even though `opencode` (reachable via `/usr/bin`) did. Added `currentNodeBinDir()` (`path.dirname(process.execPath)`, nvm-agnostic — no hardcoded paths/usernames/versions) and exactly one new read-only bind (`--ro-bind`, never writable) for that directory, prepended to PATH; no-ops cleanly (no bind, unchanged PATH) when the directory is absent or unreadable, so non-nvm hosts are unaffected. Security self-review confirmed: read-only, `--clearenv` still wipes everything first, no other containment property widened, bound-directory contents audited (Node CLI wrappers only, nothing that grants broader reach). Empirical containment tests re-ran live (bwrap was available) and still pass unmodified; one new positive-case test confirms `node` actually resolves and runs inside a real sandboxed session. **Important open finding, deliberately not fixed:** `claude`'s own binary is a symlink into the sibling `lib/node_modules/...` directory, which is NOT bound — so `claude` is still unreachable inside the sandbox even after this change (a dangling symlink), while `node` itself now works. Fixing that requires binding the much broader `lib/` tree (exposes every globally-installed npm package), which is a further security-boundary decision that needs explicit human approval, not something to fold in silently.

**Verification across all of the above:** full suite went from 225 (start of session) → 272 passing (40 test files), `npx tsc --noEmit` clean, `npm run build` clean (same pre-existing chunk-size advisory) at every commit boundary. `mcp__gitnexus__detect_changes` was run before every commit; the GitNexus index was stale for the whole session (last indexed before today's commits) — every result was cross-verified against `git diff`/the test suite rather than trusted blindly, consistent with how prior sessions in this doc handled the same staleness.

**Not done in this pass, flagged for follow-up (do not silently pick these up without a human decision first):**
- Live Phase 1 test matrix in `docs/llm-provider-testing-plan-2026-07-01.md` (Council mode, rate-limit fallback chains, large-doc OKF budget) — needs a human with real provider API keys in-session.
- `interview.ts`'s DAG proposal generation is template-based, not LLM-driven — a real architectural decision, not a quick fix.
- `UnifiedMasterDashboard.tsx`'s Log Level setting persists but is never enforced anywhere in the backend.
- `claude` CLI still cannot launch inside the embedded terminal (dangling `lib/` symlink target) — needs an explicit decision on whether to widen the sandbox bind further.
- Server-side `ScopeLogicSession` (`src/server/index.ts`) is a single global instance regardless of active project — a deeper multi-tenancy gap noted during Task 16, not fixed.
- Two remaining low-priority items from the 06-30 audit's own action plan were never picked back up: the route-layer split (`portgrid-router.ts` vs `coreexec-router.ts`, Task 5 step 4) and mirroring the Deference UI split onto `UnifiedMasterDashboard.tsx` (Task 3 step 6) — both explicitly optional/deferred twice already.

### 2026-07-03 (same day, second pass) — live LLM testing, LLM-driven proposals, full terminal CLI usability, real log levels

Continuation of the same day's dashboard reality pass, after a follow-up interview with the user covering 5 more decisions: live-test the LLM providers with real keys, make DAG proposal generation LLM-driven, decide the fate of `claude`/`codex`/`agy` in the terminal, decide on Log Level enforcement, and fix ScopeLogic's session scoping. The user chose the fuller option on every item, including the single highest-stakes decision of the day: full, unrestricted network access for the embedded terminal.

**Task 19 — live LLM-provider testing with real keys.** The user provided real OpenCode Zen and OpenRouter API keys directly in-session (never written to any file, only ever passed inline to `curl`/registered via the app's own `PUT /api/llm/providers/:id` — which encrypts at rest — exactly like a user would through the UI). Findings:
- **OpenCode Zen works, with a caveat.** A short prompt ("what is a DAG?") succeeded cleanly through Cerebro chat. A longer/more complex prompt **consistently failed** with `LLM API returned no content in response` (3/3 reproductions), falling through the full RouteSwitch fallback chain (OpenRouter → FreeLLMAPI → local llama-cpp). Suspected cause, not yet fixed: `server/index.ts`'s Cerebro `generateFn` requests only `estimatedTokens: 150` — likely too low once the free model's own reasoning-token overhead is accounted for on longer prompts. Flagged for follow-up, not blindly patched.
- **OpenRouter failed with a real, informative error**: `"No endpoints available matching your guardrail restrictions and data policy. Configure: https://openrouter.ai/settings/privacy"` — this is an account-level privacy/data-policy setting on OpenRouter's own side (their free-tier endpoints require opting into prompt logging), not a bug in this codebase. The user needs to visit that URL themselves.
- **FreeLLMAPI**: correctly skipped per user instruction (its proxy server is down); the fallback chain treats it as unavailable and moves on cleanly.
- Live-verified in-browser: `CoreExecDashboard`'s real metrics (Task 15) reflected genuinely live data (Active Runs count moved when test todos were seeded); PortGrid's unified approval queue (Task 16) correctly showed a live-seeded 0.35-confidence item in "Attention Required" and a 0.90-confidence item in the Deference pill bar simultaneously, and both the pill bar's "Approve All" and the individual "Decline" button (Task 13) were clicked live and confirmed via direct DB check to flip `status` to `resolved`/`rejected` respectively.
- The two verified-working keys were applied both to a disposable worktree test copy of the database AND to the user's real `.data/neurosync.db` (via the same `PUT`/`test` API endpoints against a briefly-started server on the original checkout — never a raw file copy over the user's real DB, which the environment's own safety layer correctly refused when first attempted as a wholesale file overwrite).

**Task 20 — full terminal CLI usability, including restoring network (the highest-stakes change of the day).** The user clarified their actual goal for the embedded terminal: it exists so they never have to leave NeuroSync to run other coding agents (`claude`, `codex`, `agy`, `opencode`, `freebuff`), and NeuroSync should passively learn from that usage (Task 17's auto-scan), not gatekeep it. Two problems stood between that goal and reality:
1. Even with Task 18's Node-bin-dir bind, `claude`/`codex` still didn't actually run — their launchers in `bin/` are symlinks into a sibling `lib/node_modules/...` directory that wasn't bound, and `agy` (a real, non-symlinked file under `~/.local/bin`) wasn't reachable at all.
2. Real CLI agents fundamentally need outbound network access to their own APIs to do anything beyond printing `--version` — the terminal's `--unshare-net` made that structurally impossible, independent of any PATH/bind fix.

The user was told plainly what removing `--unshare-net` means (full, unrestricted network from an already-unrestricted shell, the same as running outside any sandbox) and explicitly approved it, with reasoning captured verbatim in this session. An Opus agent (per the plan's model-routing for security-sensitive work) then: removed `--unshare-net` entirely; added two new read-only binds (the nvm `lib/node_modules` sibling directory, generically derived — no hardcoded paths/versions — and `~/.local/bin`); rewrote the file's header comment to document network-openness as a deliberate decision, not a regression; re-ran the full empirical containment suite live (bwrap present) confirming filesystem confinement is completely unchanged (`/etc/shadow` still denied, paths outside the project still not found); and added a live positive-network test. Verified hands-on in-browser afterward: inside a real terminal session for the `BBR Pipeline` project, `pwd` confined correctly, all 5 tools (`claude`, `codex`, `agy`, `opencode`, `freebuff`) resolved via `which`, a real HTTPS request to `api.anthropic.com` succeeded (genuine TLS handshake, HTTP 404 response), and **`claude --version` actually launched and printed `2.1.199 (Claude Code)`** — the exact thing that was broken before this task. `PortGridDashboard.tsx`'s user-facing description text (which still said "network access removed") was also corrected to match.

**Task 21 — LLM-driven DAG proposal generation + per-project sessions, done by an Opus agent.** Task 15 had surfaced that `interview.ts`'s `_generateProposal()` was template-based (substring truncation), not an LLM call, unlike the genuinely LLM-driven conversational Q&A leading up to it — meaning `dag_proposals.confidence` (from Task 16) always got a hardcoded 0.5 with nothing real behind it. Fixed by reusing the exact pattern `OKF/generator.ts` already established for schema-constrained generation (a `responseSchema` hint threaded through `RouteSwitchEngine.execute()`, not a new mechanism): the LLM now designs the actual DAG from the real interview transcript and self-reports a `confidence` value in its structured JSON response, validated by the same `ValidatorLogic.validate()` safety gate that already existed, with the original template generator retained as the fallback for any failure (no `generateFn`, LLM error, unparseable response, or validator rejection) — the conversational path already degraded this way, so proposal generation now matches. Separately, `server/index.ts`'s single global `ScopeLogicSession` (shared across every project, an inconsistency the plan noted) was replaced with per-project session scoping via a new `scopelogic-router.ts` extraction (matching this codebase's existing router-file convention) and a capped, LRU-evicted session map. +10 tests, 289/289 passing after this task.

**Task 22 — real Log Level enforcement.** `UnifiedMasterDashboard`'s Log Level control (surfaced as inert during Task 15's telemetry pass) saved a setting nothing ever read. Built `src/core/observability/logger.ts` (`log.debug/info/warn/error`, gated by `system_settings.log_level`, cached 2s) and migrated the 9 highest-traffic `console.*` call sites (66 of ~99 repo-wide) — `server/index.ts`, `coreexec/scheduler.ts`, `routeswitch/engine.ts`, `routeswitch/router.ts`, `scoutdaemon/idle.ts`, `memory/cerebro/reflection.ts`, `routes/system.ts`, `routes/cerebro.ts`, `portgrid/terminal-session.ts`. Deliberately did NOT migrate `basevault/db.ts` (it's the module `logger.ts` itself imports, and it passes `console.log` as its own `verbose` callback during construction — migrating it would create a circular import). Live-verified against the real copied dev database: `log_level` was already set to `'warn'` from a genuine prior session (a previously-inert real user setting), and after this change boot-time `info` logs from migrated files were correctly suppressed at that level while unmigrated files were unaffected — exactly the intended, deliberately partial scope. 295/295 tests passing after this task.

**Verification across this whole second pass:** suite went 279 (task 19 start, no code change) → 289 (task 21) → 295 (task 22) passing; `npx tsc --noEmit` and `npm run build` clean at every commit boundary; GitNexus `detect_changes` run before every commit (index stale all day, as flagged repeatedly — cross-verified against `git diff`/tests each time, consistent with how the rest of this session handled the same staleness).

**Not done, flagged for follow-up (do not silently pick up without asking first):**
- OpenCode Zen's intermittent "no content in response" failure on longer prompts (likely `estimatedTokens: 150` being too low for Cerebro chat once reasoning-token overhead is accounted for) — a real, reproducible finding, not yet fixed.
- OpenRouter needs the user to configure their own account's data/privacy policy at `openrouter.ai/settings/privacy` before its free endpoints will work through this app — nothing to fix in code.
- Two harmless "ScoutDaemon Discovery" project rows seen mid-session in the UI switcher turned out to be stale frontend cache from the session's own test `/promote` calls (already cleaned up server-side) — confirmed via direct DB query that zero such rows actually exist; no action needed.
- A pre-existing `ScoutDaemon` idle-maintenance `FOREIGN KEY constraint failed` error was observed in server logs during this session (`idle.ts`'s synthetic maintenance-DAG creation) — unrelated to any change made today, not investigated further.

### 2026-07-03 (same day, third pass) — 24-scenario reality test against the live marketing site's claims

Trigger: a request to test every real-world thing a user might want to do (project management, learning, scheduled agent loops) through the actual UI, with at least 20 concrete scenarios, and check whether the app delivers on the specific promises at `https://neurosync.bytebuster-tools.online`. Fetched the marketing page directly and extracted its literal claims before designing scenarios, so this tested promises, not just clicked around. 24 scenarios across three categories (A: project management ×9, B: learning ×6, C: scheduled research/agent loops ×9), executed against the live worktree app (all 13 commits from today's earlier two passes). Full report with severity-coded findings published as an artifact; summary below.

**Operational lesson learned the hard way:** the first attempt ran three parallel test-fork batches (A/B/C) against one shared SQLite-backed dev server simultaneously. This caused real cross-contamination — one fork's "BBR Pipeline" project got renamed to "refresh-stale" with its path cleared by another fork's edit-path test, and the LLM provider registry was wiped/reset mid-test by yet another fork's Free-Mode-Governor testing. Both were caught (the project rename via a direct DB check after the environment's own safety layer blocked a related destructive action, the provider wipe via a stale-provider-ID mismatch in a later chat response) and fixed before they reached anything real — this only ever touched a disposable worktree test copy (`.claude/worktrees/dashboard-reality-pass/.data/neurosync.db`), never the user's actual `.data/neurosync.db`. The Learning batch (B) additionally stalled twice (once past a 600s watchdog, once returning a degenerate non-answer) before being completed by direct hands-on testing in the main session instead of a sub-agent. **Lesson for future sessions: don't run multiple parallel test agents against one shared mutable dev database — sequence them, or give each its own isolated DB copy.**

**Direct marketing-claim verdicts** (the highest-value output of this pass):
- **"Free Mode Governor... blocks paid-provider calls unless explicitly unlocked" — FALSE as described.** `FreeModeGovernor` is a flat 100,000-token cumulative ceiling with zero paid/free provider distinction and zero unlock mechanism anywhere in the codebase (grepped exhaustively: no UI control, no API route, no setting). Confirmed the block itself is real (triggered live with a 106,250-token proposal, got the exact governor error). Only a full process restart resets the counter.
- **"if it crashes mid-task, it resumes from the last completed step" — FALSE.** Nothing scans for in-progress (`status='running'`) runs on server boot. `POST /api/coreexec/retry/:runId` exists but filters `WHERE status IN ('failed','parked')` — a task that was `'claimed'` (actively executing) at the moment of a real crash is not covered and would be permanently stuck with no automatic or manual path forward.
- **New finding surfaced while testing crash recovery (not on the original scenario list): most real DAG proposals would execute as no-ops.** `src/core/coreexec/dispatch.ts` only has real dispatch logic for `'shell'` (allowlisted commands) and `'scrape'` (URL fetch) task shapes. Everything else — which is what a natural-language task description (exactly what ScopeLogic's now-LLM-driven proposal generation produces) looks like — falls through to `'generic'`, which the code's own comment labels `metadata echo (legacy/no-op fallback)`: it marks the task complete and returns a canned string without ever calling an LLM to do the described work. This directly contradicts "CoreExec... Runs actual multi-step workflows."
- **"Prevents duplicate side effects" — confirmed TRUE.** Retried an already-fully-completed 3-task run; all tasks and the run stayed untouched, no re-execution.
- **"Nothing acts without your approval... every AI action waits here until you approve it" — confirmed TRUE.** Live-verified the merged Deference UI queue (mixed confidence levels, correct split), and confirmed the cron-scheduling path validates through the same SA-07 safety gate as the interactive approval path before a job can ever go live.
- **"Automatic redaction of anything sensitive" — inconclusive.** Attempted with a fake credential through Cerebro chat; the LLM provider chain was failing during the test window (see below), so the redaction code path was never actually exercised. Needs a clean re-run.

**Real bug found and fixed this pass:** `GET /api/basevault/runs` had zero `project_id` filter — confirmed live that a brand-new, empty test project immediately displayed a *different* project's workflow run in its "Active Workflow Runs" widget. Affected three separate dashboards (`CoreExecDashboard.tsx`, `BaseVaultDashboard.tsx` — whose own comment already said "Fetch runs (project-scoped)" while never actually doing so, and `PortGridDashboard.tsx`), all of which re-fetch on `activeProjectId` change but never sent it. Fixed with an optional `?projectId=` query param (omitting it preserves `RunHistory.tsx`'s legitimate all-projects view), verified live by seeding one run per project across two different projects and confirming filtered vs. unfiltered results differ correctly. Committed and pushed.

**Other real findings from the 24 scenarios:**
- A newly-registered LLM provider (OpenCode Zen's free model) is **confirmed non-deterministic**: an identical short prompt succeeds once, then fails 3/3 in a row minutes later with "no content in response" — worse specifically on code-classified chat messages. This corroborates and sharpens the intermittent-failure finding from the second pass earlier today.
- The auto-scan-on-terminal-close learning loop (built earlier today) has a real edge case: two terminal sessions for the same project closing within the 60-second cooldown window means the second session's real file changes can be silently missed, because the cooldown guard is purely time-based and doesn't check whether new changes actually exist. A manual scan immediately after correctly found the missed file.
- Editing a project's root path (simulating a moved folder) writes a fresh `neurosync-config.yaml` at the new location but leaves the stale one behind at the old path.
- GovernorUI's CPU-temp display can still show a misleadingly-real-looking `0°C` — today's earlier fix (Task 13) only special-cased negative/`-1` sensor-failure readings, not a literal `0` return, which this environment's `systeminformation` call apparently produces.
- Cerebro's reflection/consolidation mechanism is confirmed genuinely real (a pre-existing historical "Consolidated new preference" log entry proves it has worked before), but this session's specific fresh-preference test was blocked by the same provider flakiness, not re-verified end-to-end today.

**Not done, flagged for a human decision before any of these get touched:**
- Free Mode Governor's actual behavior doesn't match its marketing description — needs a decision on which side to fix (build the paid/free distinction + unlock mechanism, or correct the site's wording).
- No real crash recovery exists — needs a boot-time scan for in-progress runs plus extending the retry endpoint to cover `claimed`-status tasks. This is real architecture work, not a quick patch.
- Most real task prompts silently no-op in CoreExec — needs a decision on whether `dispatch.ts` gets a real LLM-executed path for non-shell/scrape prompts, or ScopeLogic's proposal generation gets constrained to only emit node shapes CoreExec can actually run. This is a significant, previously-undiscovered gap between the two features built earlier today (LLM-driven proposal generation + the unified approval queue) and the execution engine underneath them.
- OpenCode Zen's free-model reliability issue needs either a retry/token-budget investigation or an explicit "known free-tier limitation" note.
- The auto-scan cooldown's blind spot (time-based, not change-aware) is a small, contained fix if wanted.
...
### 2026-07-04 (continued) — fixed the two remaining marketing-claim gaps: Free Mode Governor and crash recovery

Direct follow-up to the reality-test pass's remaining priority items, both done by Opus agents per the plan's model-routing, run sequentially (not in parallel) to avoid repeating the earlier concurrent-fork database contention.

**Free Mode Governor — now real.** `neurosync.bytebuster-tools.online` claims it "blocks paid-provider calls unless explicitly unlocked"; it was actually just a flat 100,000-token ceiling with no paid/free distinction and no unlock mechanism. Fixed with a deliberately opt-in design: a new `llm_providers.is_paid_tier` column (`DEFAULT 0` for every row, including existing ones — nothing gets silently reclassified as paid by provider `type`, since guessing wrong would have immediately and silently blocked the user's own working free-tier setups) plus a global `system_settings.free_mode_unlocked` lock (absent/anything-else = locked, the safe default). Enforcement skips a paid+locked provider in the fallback chain exactly like an exhausted one — a free provider later in the chain still serves the request, it's not a hard failure. Also fixed a latent pre-existing bug found along the way: when every candidate in a chain was skipped, the loop previously fell through and returned an undefined response instead of a clear error. UI: a checkbox on the Add/Edit Provider form ("This provider may incur real charges"), an amber "Paid" badge on provider cards, and a Locked/Unlocked toggle in RouteSwitchDashboard's Governor section. +10 tests (309 total after this task). **Live-verified end to end** against the real running dev server: marked OpenCode Zen (the primary provider) paid while locked, confirmed the request fell through to OpenRouter instead of using it; unlocked, confirmed OpenCode Zen served a real request again. Commit `ef0f33f`.

**Crash recovery — now real.** The same site claims "if it crashes mid-task, it resumes from the last completed step"; nothing anywhere scanned for in-progress (`status='running'`) runs at boot, so a hard crash left a run stuck forever. Turned out to be a smaller fix than expected: `executeRun()` was already idempotent and task-state-driven (each loop iteration re-reads live task state from the DB, skips already-`completed` tasks, and only re-claims a `'claimed'` task once its lease has actually expired) — so no new resumption mechanism was needed, just something to call it again after a restart. Added an exported `resumeInProgressRuns()` in `engine.ts` (queries `workflow_runs WHERE status IN ('running','pending')`, fire-and-forgets `executeRun()` for each, logs how many were found), wired into `server/index.ts` right after `initScheduler()`. Also extended `POST /api/coreexec/retry/:runId` to immediately reset a `'claimed'` task back to `unclaimed` when its lease has *already* expired (so a human doesn't have to wait out the remaining timeout to manually recover a stuck run) — deliberately left untouched a `'claimed'` task whose lease is still valid, since resetting it would break the lease's mutual exclusion and let a second process race one that may still legitimately be in flight. +3 tests (312 total after this task).

**This is the one claim in the whole day's work that got a real, non-negotiable empirical proof, not just a code read:** approved a real 2-node DAG (`pwd` → a deliberately slow script), waited until node A was genuinely `completed` and node B was genuinely `claimed`/in-flight, then `kill -9`'d the entire server process tree (a real hard crash, not a graceful shutdown) and restarted it. The boot log printed `"found 1 in-progress run(s) ... resuming from last completed step"`; node A's `output_data` was confirmed byte-identical before and after (proving it was NOT re-executed); the engine correctly waited out node B's still-valid lease before reclaiming it; and the run reached a terminal state entirely on its own, with no manual intervention. This is the strongest evidence-based confirmation any single fix got today.

**Operational note:** the first attempt at this task got interrupted mid-verification when the session hit its usage limit while the agent was mid-wait on the real crash test's timing (lease expiry + re-run completion). The agent had already committed its work (`f2fbb00`) before being cut off, so nothing was lost — resuming in a fresh session found the commit already in place with a complete, honest report of the real test results, and independent re-verification (fresh `npm test`/`tsc`/`build`) confirmed everything the agent reported. **Lesson: a commit-before-you-wait discipline paid off here — don't leave uncommitted work sitting behind a long real-world timing test.**

**Verification across both fixes:** suite went 299 (start of this section) → 309 (Governor) → 312 (crash recovery); `npx tsc --noEmit` and `npm run build` clean at every commit boundary; GitNexus `detect_changes` reviewed before every commit (index stale all day, as flagged repeatedly — cross-verified against `git diff`/tests each time).

**Still open from the reality-test pass:**
1. OpenCode Zen's free-model reliability issue (confirmed non-deterministic empty-content failures) needs investigation or a documented known-limitation note.
2. The terminal auto-scan cooldown is time-based, not change-aware, and can miss real changes if two sessions close within 60 seconds of each other.
3. A7 (redaction log) and A8 (archive-with-history) from the reality test are still inconclusive — worth a clean re-run once a provider is behaving reliably.
4. C3/C4 (idle-triggered ScoutDaemon maintenance / reflection start-stop) still need a genuine 10-minute idle observation window.
5. Council Mode does not filter paid providers from the new Governor lock (found during the Governor fix, deliberately left alone — filtering could drop council below its 2-provider minimum; only affects high-risk prompts with ≥2 explicitly-configured council providers).

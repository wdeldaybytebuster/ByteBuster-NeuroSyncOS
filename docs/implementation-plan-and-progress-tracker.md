...
| 18 | Bind current Node bin dir into terminal sandbox (nvm-installed CLI reachability) | P1 | VERIFIED (tests + typecheck + build + empirical containment re-proof) — done by Opus per plan; `claude` still blocked by an unbound `lib/` symlink target, flagged as open follow-up | 2026-07-03 |
